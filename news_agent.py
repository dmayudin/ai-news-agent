...
class NewsAgent:
    ...
    def analyze_with_ai(self, news_items: List[Dict]) -> str:
        ...
        prompt = f"""Ты AI-аналитик медиа. Проанализируй новости об ИИ и напиши живую, интересную сводку на русском языке.

НОВОСТИ:
{news_text}

Напиши сводку строго в этом формате (используй эмодзи):

🔥 ТОП-3 СОБЫТИЯ:
1. <название> — <почему важно, 1-2 предложения>
2. <название> — <почему важно>
3. <название> — <почему важно>

📈 ГЛАВНЫЕ ТРЕНДЫ:
• <тренд 1 — 1 предложение>
• <тренд 2 — 1 предложение>

💡 ИНТЕРЕСНО:
• <факт или инсайт — 1 предложение>
• <факт или инсайт — 1 предложение>

Пиши живо, как для умного читателя. Переводи заголовки на русский. Без воды и канцелярита."""
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "Ты эксперт-аналитик в области ИИ. "
                            "Пишешь умно, живо, по делу — на отличном русском языке. "
                            "Переводишь все заголовки и термины на русский. "
                            "Никакого канцелярита и воды."
                        )
                    },
    ...
    def translate_title(self, title: str) -> str:
        """Переводит заголовок на русский язык если он на английском."""
        ru_chars = sum(1 for c in title if '\u0400' <= c <= '\u04FF')
        if ru_chars / max(len(title), 1) > 0.3:
            return title  # уже на русском
        try:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "Ты переводчик технических новостей об ИИ. "
                            "Переводи точно, живым русским языком. "
                            "Сохраняй технические термины (AI, LLM, GPT, etc.) как есть. "
                            "Отвечай ТОЛЬКО переводом заголовка, без кавычек и пояснений."
                        )
                    },
                    {"role": "user", "content": f"Переведи заголовок: {title}"}
                ],
                temperature=0.2,
                max_tokens=100
            )
    ...
    def generate_post(self, news_items: list) -> str:
        ...
        for i, item in enumerate(top, 1):
            ...
            news_text += "\n"
        prompt = f"""Напиши один пост для Telegram-канала об ИИ на основе этих новостей.
...
Требования:
- Длина: 150-250 слов
- Стиль: живой, умный, без кликбейта
- Структура: короткий заголовок с эмодзи → 2-3 абзаца → вывод/мысль
- Язык: отличный русский, переводи заголовки
- В конце добавь 3-5 тематических хэштегов

Пиши как умный человек для умных читателей."""
        try:
            return chat_complete(
                messages=[
                    {"role": "system", "content": "Ты редактор Telegram-канала об ИИ. Пишешь умно, живо, без воды."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.8,
                max_tokens=600
