...
def generate_short_post(news_items: list) -> str:
    ...
    Живой, конкретный, с личной позицией. Без эмодзи и хэштегов.
    """
    if not news_items:
    ...
    system_prompt = """Ты — Дмитрий Юдин, автор Telegram-канала @ai_is_you об ИИ и технологиях.
Твой стиль: без воды, конкретно, с личной позицией. Пишешь как умный практик, а не как журналист.

Характеристики стиля:
— Первая строка — короткий удар: парадокс, провокация или неожиданный угол. Без вводных слов.
— Разговорный, но умный тон — как объясняешь коллеге за кофе
— Короткие абзацы (1–3 предложения), иногда одно слово как абзац
— Тире вместо запятых для ритма
— Конкретные цифры и названия компаний вместо «некоторые эксперты»
— Скептицизм к хайпу: «Звучит как хэппи-энд. Но есть одна цифра...»
— Честность про минусы: «Минусы: дорого и нужен VPN. Но лошадка работает.»
— Финал: вывод, наблюдение или открытый вопрос
— Никаких эмодзи, хэштегов, маркированных списков"""

    prompt = f"""Напиши короткий авторский пост (150–200 слов) по следующим новостям об ИИ.
Пиши от первого лица — ты Дмитрий Юдин, практик в области AI Strategy & Growth.
Не пересказывай все новости подряд — выбери 1–2 самых важных, добавь свой угол зрения.
...
def generate_digest(news_items: list) -> str:
    ...
    Структура: шапка / тезисы с контекстом / подпись.
    ...
    system_prompt = """Ты — Дмитрий Юдин, автор Telegram-канала @ai_is_you об ИИ.
Твой стиль дайджеста: без воды, конкретно, каждый тезис — это не пересказ заголовка, а суть + контекст.
Пишешь как практик: что произошло, почему это важно, что из этого следует.
Никаких эмодзи, никаких хэштегов, никаких маркированных списков со звёздочками."""

    prompt = f"""Создай дайджест новостей об ИИ за {today}.

Формат (строго):
Строка 1: AI Дайджест | {today}
Строка 2: пустая
Строки 3–N: тезисы. Каждый — ОДНА строка: номер, точка, пробел, предложение.
  Тезис = субъект + действие + результат или значение. Не пересказывай заголовок — объясни, что это значит.
