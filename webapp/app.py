...
def api_generate():
    ...
    try:
        ...
        # Берём переведённые заголовки и описания
        items = news[:15]
        trs   = translate_batch(items)
        formatted_items = [fmt_item(n, trs[i]) for i, n in enumerate(items)]

        news_block = '\n'.join([
            f"- {it['title']}: {it['summary'][:150]}"
            for it in formatted_items if it['title']
        ])
        ...
        if gen_type == 'post':
            prompt = (
                ...
                'Без хэштегов. Без эмодзи. Только текст.\n\n'
                'Новости:\n' + news_block
            )
        else:  # digest
            prompt = (
                ...
                'Формат: заголовок дайджеста, затем 5–7 пунктов с кратким описанием каждой новости (1–2 предложения). '
